...
test('Macros renders retained commands and the complete operational action set', () => {
  const markup = renderToStaticMarkup(<MacrosPage runtime={{
    ...
    library: { version: 2, macros: [{
      ...
      name: 'Lights', risk: 'safe', steps: [{ type: 'command', commandId: 'command.elite.ShipSpotLightToggle', operation: 'tap' }], version: 2
    }] },
    play: vi.fn(), recordCommand: vi.fn(), save: vi.fn(), setDraft: vi.fn(), startRecording: vi.fn(), stopRecording: vi.fn()
...
test('Macros exposes draft timing and playback abort without claiming game outcome', () => {
  const markup = renderToStaticMarkup(<MacrosPage runtime={{
    ...
    draft: {
      ...
      entries: [{ commandId: 'command.elite.ShipSpotLightToggle', delayBeforeMs: 250, message: 'Accepted.', operation: 'tap', status: 'accepted' }]
    },
    library: { version: 2, macros: [] },
