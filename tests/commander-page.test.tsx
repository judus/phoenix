...
import { CommanderLoadoutsPage } from '../apps/web/src/features/equipment/commander-loadouts-page.js'
import { EquipmentPage } from '../apps/web/src/features/equipment/equipment-page.js'
import { equipmentNavigationItems } from '../apps/web/src/features/equipment/equipment-navigation.js'
...
test('commander family exposes typed contextual destinations', () => {
  expect(commanderNavigationItems.map(item => [item.label, item.href])).toEqual([
    ...
    ['Suit loadouts', '#/commander/loadouts'],
    ['Statistics', '#/commander/statistics']
  ])
})

test('commander loadouts and equipment gear share observed state without duplicating page ownership', () => {
  const equipment = {
    schemaVersion: 1 as const,
    ownershipCoverage: 'observed' as const,
    currentLoadoutId: 7,
  ...
  const loadouts = renderToStaticMarkup(
    <CommanderLoadoutsPage controller={{ status: 'ready', equipment }} />
  )
  ...
  expect(loadouts).not.toContain('<h2>Suits</h2>')
  expect(loadouts).not.toContain('<h2>Personal weapons</h2>')
  expect(gear).toContain('<h1>Gear</h1>')
  expect(gear).toContain('<h2>Suits</h2>')
  expect(gear).toContain('<h2>Personal weapons</h2>')
  expect(gear).toContain('Artemis Suit')
  expect(gear).toContain('Manticore Executioner')
