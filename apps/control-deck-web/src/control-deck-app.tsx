...
export function DeckButton ({ editing, element, label, onEdit, onTap, onHoldStart, onHoldEnd }: { editing: boolean, element: ControlDeckCommandElement, label: string, onEdit(): void, onTap(): void, onHoldStart(): void, onHoldEnd(): void }) {
  ...
  const finishPointer = () => {
    ...
    if (!editing && element.interaction.confirmation.kind === 'none' && element.interaction.activation === 'hold') onHoldEnd()
  }
  return <button className={`deck-button${armed ? ' armed' : ''}`} onClick={activate} onContextMenu={event => event.preventDefault()} onPointerDown={event => {
    if (editing) return
    ...
    } else if (element.interaction.activation === 'hold') onHoldStart()
...
function ButtonEditor ({ element, position, onClose, onSave, onRemove }: { deck: ControlDeckDeck, element?: ControlDeckCommandElement, position: { column: number, row: number }, onClose(): void, onSave(element: ControlDeckCommandElement): void, onRemove(): void }) {
  ...
  const [modifiers, setModifiers] = useState(Array.isArray(element?.target.configuration.modifiers) ? element.target.configuration.modifiers.join('+') : '')
  ...
  return <section aria-modal="true" className="button-editor" role="dialog"><header><strong>Button {position.column}:{position.row}</strong><button onClick={onClose}>Close</button></header><label>Label<input autoFocus value={label} onChange={event => setLabel(event.target.value)} /></label><label>Keyboard key<input value={key} onChange={event => setKey(event.target.value)} placeholder="Space, F1, A…" /></label><label>Modifiers<input value={modifiers} onChange={event => setModifiers(event.target.value)} placeholder="Ctrl+Shift" /></label><label>Activation<select value={activation} onChange={event => { const next = event.target.value as 'tap' | 'hold'; setActivation(next); if (next === 'hold') setConfirmation('none') }}><option value="tap">Tap</option><option value="hold">Hold while pressed</option></select></label><label>Safety<select disabled={activation === 'hold'} value={confirmation} onChange={event => setConfirmation(event.target.value as 'none' | 'arm-then-tap')}><option value="none">Immediate</option><option value="arm-then-tap">Hold to arm, then tap</option></select></label><footer>{element && <button className="danger" onClick={onRemove}>Remove</button>}<button disabled={!key.trim() || !label.trim()} onClick={() => onSave({ id: element?.id ?? `element_${Date.now().toString(36)}`, kind: 'command', target: { adapterId: 'builtin.keyboard', commandId: 'key', configuration: { key: key.trim(), modifiers: modifiers.split('+').map(value => value.trim()).filter(Boolean) } }, placement: element?.placement ?? { kind: 'grid', column: position.column, row: position.row, columnSpan: 1, rowSpan: 1 }, appearance: { label: label.trim(), icon: null, foregroundColor: null, backgroundColor: null }, interaction: { activation, confirmation: confirmation === 'arm-then-tap' ? { kind: confirmation, armedForMs: 5_000 } : { kind: 'none' } } })}>Save button</button></footer></section>
}
