...
import { EqualGrid, IconButton, Metric, Stack } from '@phoenix/ui'
import type {
...
export function DashboardRadioControls({
  ...
  const execute = async (action: GameActionAvailability): Promise<void> => {
    ...
    try {
      ...
      setResult({
        failed: true,
        message: cause instanceof Error ? cause.message : 'GalNet Audio command failed.'
      })
    } finally {
  ...
  return (
    <Stack gap="sm" fill justify="space-between">
      ...
      <EqualGrid columns={4} gap="xs" aria-label="GalNet Audio controls">
