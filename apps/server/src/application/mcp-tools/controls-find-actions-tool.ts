...
export class ControlsFindActionsTool implements LocalTool {
  ...
  public readonly execute = (arguments_: JsonObject) => {
    ...
    const groupId = optionalStringArgument(arguments_, 'groupId')
    ...
    const matches = this.commands.getCatalog().commands
      ...
      .filter(command => groupId === undefined || command.groupId === groupId)
      .filter(command => {
        const haystack = [command.id, command.label, command.description, command.groupId, command.kind].join(' ').toLowerCase()
