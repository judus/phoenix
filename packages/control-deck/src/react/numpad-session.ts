import { numpadChildren, resolveNumpadLevel, type NumpadTreeNode, type NumpadTreeSnapshot } from '../core/index.js'
...
export function enterNumpadDigit (snapshot: NumpadTreeSnapshot, state: NumpadSessionState, digit: string, alwaysConfirm: boolean): NumpadSessionTransition {
  ...
  if (resolution.status === 'ready' && resolution.exact) return chooseNode({ ...active, pendingDigits: digits }, resolution.exact, alwaysConfirm, false)
...
export function confirmNumpadSelection (snapshot: NumpadTreeSnapshot, state: NumpadSessionState): NumpadSessionTransition {
  ...
  return resolution.exact ? chooseNode(state, resolution.exact, false, true) : { state: { ...state, status: 'invalid', message: 'No exact command matches this entry.' } }
}
...
export function selectNumpadNode (snapshot: NumpadTreeSnapshot, state: NumpadSessionState, nodeId: string, alwaysConfirm: boolean): NumpadSessionTransition {
  ...
  return node ? chooseNode(state.active ? state : activateNumpadSession().state, node, alwaysConfirm, false) : { state: { ...state, status: 'invalid', message: 'Command node is unavailable.' } }
}
...
function chooseNode (state: NumpadSessionState, node: NumpadTreeNode, alwaysConfirm: boolean, confirmed: boolean): NumpadSessionTransition {
  if (node.kind === 'menu') return { state: { active: true, pathIds: [...state.pathIds, node.id], pendingDigits: '', status: 'browsing' } }
  if (!node.available || !node.commandId) return { state: { ...state, pendingDigits: node.selector, readyNodeId: node.id, status: 'unavailable', message: node.unavailableReason ?? 'Command is unavailable.' } }
