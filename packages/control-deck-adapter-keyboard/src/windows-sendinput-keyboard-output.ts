...
export interface WindowsSendInputKeyboardOutputOptions {
  environment?: NodeJS.ProcessEnv
...
export class WindowsSendInputKeyboardOutput implements KeyboardOutput {
  ...
  public constructor (options: WindowsSendInputKeyboardOutputOptions = {}) {
  ...
  public async send (operation: ControlDeckCommandOperation, binding: KeyboardCommandConfiguration, signal?: AbortSignal): Promise<void> {
...
function windowsInputEvents (operation: ControlDeckCommandOperation, binding: KeyboardCommandConfiguration): WindowsInputEvent[] {
  const keys = [
...
public static class ControlDeckSendInput {
...
while ($null -ne ($request = [Console]::In.ReadLine())) {
    try {
        ...
        [ControlDeckSendInput]::Send($virtualKeys.ToArray(), $flags.ToArray(), $holdMilliseconds)
